...
def load_marker_positions_txt(path: str | Path):
    """
    Returns:
      gt:   [T, J, 3] float32 (global translations)
      names: list[str] of joint names (length J), expected to match SMPLH_MUJOCO_NAMES order
      fps: float or None (if not encoded in the file)
    """
    path = Path(path)

    # Try reading header to detect layout
    with path.open("r", newline="") as f:
        header_line = f.readline().strip()
    header = [h.strip() for h in header_line.lstrip("#").split(",")]

    is_long = ("name" in header) and ("x" in header) and ("y" in header) and ("z" in header)

    if not is_long:
        # ---- WIDE LAYOUT ----
        # cols: frame,time_s,<name1>_x,<name1>_y,<name1>_z,...
        arr = np.loadtxt(path, delimiter=",", skiprows=1)
        # pull names from header
        name_cols = header[2:]
        assert len(name_cols) % 3 == 0, "Header must have groups of 3 per marker."
        names = [c[:-2] for i,c in enumerate(name_cols) if i % 3 == 0]  # strip _x
        T, rest = arr.shape
        J = rest // 3
        gt = arr[:, 2:].reshape(T, J, 3).astype(np.float32)
        # optional fps from time column
        time_s = arr[:, 1]
        fps = None
        if T > 1:
        ...
        for r in rows:
            ...
            gt[t, j, 1] = float(r["y"])
            gt[t, j, 2] = float(r["z"])
            time_by_frame[t] = float(r["time_s"])

        fps = None
        if T > 1 and all(k in time_by_frame for k in range(T)):
            dt = np.median(np.diff([time_by_frame[k] for k in range(T)]))
            if dt > 0:
                fps = float(round(1.0/dt))
        return gt, names, fps
...
def retarget_from_markers_txt(
    markers_txt: str | Path,
    output_path: str | Path,
    robot_type: str,
    *,
    saved_was_scaled_for_viewer: bool = True,   # set to False if you saved unscaled data
    saved_robot_type: str | None = None,        # which robot_type was used when saving (for undo scale); defaults to robot_type
    override_fps: float | None = None,
    render: bool = False,
):
    # 1) Load
    gt, names, fps_from_file = load_marker_positions_txt(markers_txt)

    # 2) Make sure joint order matches the code’s expectation
    if names != SMPLH_MUJOCO_NAMES:
        # reorder to SMPLH_MUJOCO_NAMES
        name_to_idx = {n:i for i,n in enumerate(names)}
        idx = [name_to_idx[n] for n in SMPLH_MUJOCO_NAMES]
        gt = gt[:, idx, :]
        names = SMPLH_MUJOCO_NAMES[:]  # now aligned

    # 3) Undo viewer scaling/offset once if needed
    if saved_was_scaled_for_viewer:
        typ = saved_robot_type or robot_type
        scale = _RESCALE_FACTOR.get(typ, np.ones(3, dtype=np.float32)).astype(np.float32)
        z_off = float(_OFFSET.get(typ, 0.0))
        gt = gt.copy()
        gt[..., 2] -= z_off
        # protect against zero scale
        safe = np.where(scale == 0, 1.0, scale)
        gt = gt / safe[None, None, :]

    # 4) Build identity global rotations (you’re tracking positions; orientation cost is ~0)
    T, J, _ = gt.shape
    pose_quat_global = np.tile(np.array([0, 0, 0, 1], dtype=np.float32), (T, J, 1))

    # 5) Pick fps
    fps = override_fps or fps_from_file or 30.0

    # 6) Reuse your existing retarget pipeline (just like retarget_motion does internally)
    #    We copy the inner loop to accept arrays directly.
    ...
    for joint_name, retarget_info in _KEYPOINT_TO_JOINT_MAP[robot_type].items():
        orientation_base_cost = 0 if robot_type == "h1" else 0.0001
        task = mink.FrameTask(
    ...
    if render:
        viewer = mujoco.viewer.launch_passive(
            model=mj_model, data=data, show_left_ui=False, show_right_ui=False, key_callback=key_callback
        )
    else:
        from contextlib import nullcontext
        viewer = nullcontext()

    retargeted_poses = []            # [T, DOF] rpy
    retargeted_transformation = []   # [T, 7]  root pos+quat

    from loop_rate_limiters import RateLimiter
    from tqdm import tqdm

    with viewer as v:
        # init qpos from first frame
        data.qpos[:] = 0
        data.qpos[:3] = gt[0, 0]                        # Pelvis pos
        data.qpos[3:7] = pose_quat_global[0, 0]         # root orientation (identity)
        mink_configuration.update(data.qpos)
        ...
        t = 0
        pbar = tqdm(total=T, desc="Retargeting frames")
        while (render and v.is_running() or not render) and t < T:
            if not key_callback.pause:
                # A) set targets
                for i, (joint_name, retarget_info) in enumerate(_KEYPOINT_TO_JOINT_MAP[robot_type].items()):
                    body_idx = smplx_mujoco_joint_names.index(joint_name)
                    target_pos = gt[t, body_idx].copy()

                    # Apply the SAME scaling your code applies inside retarget_motion:
                    ...
                    # orientation: keep identity (very small/zero cost anyway)
                    tasks[i].set_target(mink.SE3.from_rotation_and_translation(mink.SO3.identity(), target_pos))

                # B) visualize GT sites (optional)
                for keypoint_name, keypoint in zip(smplx_mujoco_joint_names, gt[t]):
                    mid = mj_model.body(f"keypoint_{keypoint_name}").mocapid[0]
                    data.mocap_pos[mid] = keypoint

                # C) solve IK a few substeps
                for _ in range(optimization_steps_per_frame):
                    limits = [mink.ConfigurationLimit(mj_model)]
                    if robot_type in _VEL_LIMITS:
                        limits.append(mink.VelocityLimit(mj_model, _VEL_LIMITS[robot_type]))
                    vel = mink.solve_ik(mink_configuration, tasks, rate.dt, solver, 1e-1, limits=limits)
                ...
                retargeted_transformation.append(data.qpos[:7].copy())
    ...
    retargeted_poses = np.asarray(retargeted_poses)
    retargeted_transformation = np.asarray(retargeted_transformation)

    # Build motion outputs using your existing helpers
    if robot_type in ["h1", "g1"]:
        print("Creating robot motion for robot type:", robot_type)
        motion_out = create_robot_motion(
            retargeted_poses, retargeted_transformation, gt, fps, robot_type
        )
        torch.save(motion_out, output_path)
    else:
        print("Creating skeleton motion for robot type:", robot_type)
        skeleton_tree = SkeletonTree.from_mjcf(f"protomotions/data/assets/mjcf/{robot_type}.xml")
        motion_out = create_skeleton_motion(
            retargeted_poses, retargeted_transformation, skeleton_tree, gt, fps
        )
        motion_out.to_file(output_path)
